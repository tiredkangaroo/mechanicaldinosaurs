...
func buildComponents(root string, env []string) ([]component, error) {
	...
	consoleArgs, err := consoleCommand(env)
	if err != nil {
		return nil, err
	}
...
// consoleCommand builds the argv used to start the django console. if CONSOLE_CMD is set, it is run
// through "sh -c" verbatim so the caller can fully customize it (e.g. to run under uvicorn/daphne
// for ASGI, or gunicorn/waitress for WSGI). otherwise a gunicorn-based WSGI or ASGI command is
// built from the CONSOLE_* variables.
func consoleCommand(env []string) ([]string, error) {
...
func runAll(ctx context.Context, components []component, env []string, grace time.Duration) error {
	...
	for i, c := range components {
		// note: deliberately exec.Command, not exec.CommandContext. CommandContext kills only
		// the direct child (a single SIGKILL to its pid) the instant the context is done, which
		// would race with and undermine the graceful, whole-process-group SIGTERM/grace/SIGKILL
		// sequence performed in shutdownAll below.
		cmd := exec.Command(c.args[0], c.args[1:]...)
		cmd.Dir = c.dir
		cmd.Env = env
		cmd.Stdout = newPrefixWriter(os.Stdout, c.name)
		cmd.Stderr = newPrefixWriter(os.Stderr, c.name)
		// give each child its own process group so a single SIGTERM/SIGKILL reaches any
		// grandchildren it spawns too.
		...
		go func(name string, cmd *exec.Cmd) {
			...
			err := cmd.Wait()
			if runCtx.Err() != nil {
				// shutting down on purpose; not a failure
				return
			}
			if err != nil {
...
func signalProcessGroup(cmd *exec.Cmd, sig syscall.Signal) {
	...
	// negative pid targets the whole process group created via Setpgid above
...
// loadEnvFile parses a simple KEY=value env file (the format used by .env.example in this repo).
// lines starting with "#" and blank lines are ignored. values may optionally be wrapped in single
// or double quotes; a trailing "# comment" after a quoted value is stripped.
...
// mergeEnv combines the current process environment with values loaded from an env file. entries
// already present in base (the process environment) win over the file, so operators can still
// override things like PORT via systemd's Environment= directive without editing .env.
...
// prefixWriter prepends "[name] " to every line written to it, buffering partial lines until a
// newline arrives so interleaved output from multiple components stays readable.
